"""
Look up chemicals on the Chemical Identifier Resolver (CIR) database using cirpy.
"""
import json
import time
...
def read_chemnet():
    """Requires json of the chemnet database, which isn't on GitHub (it's 3GB)."""
    chem_names = []
    with open('../chemnet_database/chemicals.json', 'rb') as f:
        count = 0
        for line in f:
            if count % 100000 == 0:
