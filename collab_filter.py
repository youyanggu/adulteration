...
def get_naive_error(Y, valid, n):
	mean = Y[Y>0].mean()
	return np.sqrt(np.sum((valid * (Y - mean))**2) / n)

def get_error_rmse(Y, U, V, valid, n):
...
valid_validate = np.isfinite(matrix_validate).astype(np.float64).values
...
#lambdas = [0, 0.01, 0.05, 0.1, 0.5, 1, 5]
#rank_ = [1,2,3,4]
lambdas = [0.05]
rank_ = [3]
m, n = Y.shape
...
d = {}
for lambda_, rank in itertools.product(lambdas, rank_):
    print "Lambda: {}, rank: {}".format(lambda_, rank)
    U = 5 * np.random.rand(m, rank)
    V = 5 * np.random.rand(n, rank)

    rmse = [get_naive_error(Y, valid, len(training))]
    rmse_validate = [get_naive_error(Y_validate, valid_validate, len(validation))]
    for i in range(iterations):
        U, V = solve_iter(U, V, Y, valid, rank)
        rmse_error = get_error_rmse(Y, U, V, valid, len(training))
        rmse_validate_error = get_error_rmse(Y_validate, U, V, valid_validate, len(validation))
        #print 'Iteration #{}'.format(i+1), mse_error, mse_validate_error
        rmse.append(rmse_error)
        rmse_validate.append(rmse_validate_error)
    Y_hat = np.dot(U, V.T)
    d[(lambda_, rank)] = min(rmse_validate)
    print rmse_error, rmse_validate_error

print "Test RMSE:", get_error_rmse(Y_test, U, V, valid_test, len(testing))
